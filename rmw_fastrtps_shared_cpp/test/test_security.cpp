...
TEST_F(SecurityTest, test_log_depth)
{
  ...
  property = distribute_depth_property(policy.properties());
  EXPECT_EQ(property.name(), distribute_depth_property_name);
  EXPECT_EQ(property.value(), "10");
}

TEST_F(SecurityTest, test_profile)
{
...
TEST_F(SecurityTest, test_profile_overwrite)
{
  std::string xml_file_path = write_logging_xml(
    "<qos>\n"
...
TEST_F(SecurityTest, test_profile_invalid)
{
...
TEST_F(SecurityTest, test_all)
{
  ...
  ASSERT_EQ(policy.properties().size(), 5u);
...
{
  eprosima::fastrtps::rtps::PropertyPolicy policy;
  rmw_node_security_options_t security_options = rmw_get_default_node_security_options();
...
TEST_F(SecurityTest, test_invalid_security_root_not_enforced)
{
  eprosima::fastrtps::rtps::PropertyPolicy policy;
  rmw_node_security_options_t security_options = rmw_get_default_node_security_options();
  security_options.security_root_path = "/some/invalid/path";
...
TEST_F(SecurityTest, test_security_file_uris)
{
  ...
  eprosima::fastrtps::rtps::PropertyPolicy policy;
  rmw_node_security_options_t security_options = rmw_get_default_node_security_options();
  security_options.security_root_path = security_root.c_str();
  security_options.enforce_security = RMW_SECURITY_ENFORCEMENT_ENFORCE;
